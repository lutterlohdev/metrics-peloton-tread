<script>
	...
	function createOrUpdateChart() {
		console.info(
			'AvgPaceOverTimeChart: createOrUpdateChart called; canvas?',
			!!canvas,
			'chartReady?',
			chartReady
		);
		...
		if (chart) {
			...
			if (!ChartCtor) return;
			console.info(
				'AvgPaceOverTimeChart: creating new Chart instance, datasets:',
			...
			{#each averagePaces as ap}
				{#if ap.avgPace !== null}
					...
					>
						...
						<span style="font-size:1.1em; color:#2c3e50;">{ap.avgPace} min/mi</span>
					</div>
				{/if}
			{/each}
		</div>
