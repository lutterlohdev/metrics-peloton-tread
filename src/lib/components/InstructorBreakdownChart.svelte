<script>
	...
	function createOrUpdateChart() {
		console.info(
			'InstructorBreakdownChart: createOrUpdateChart called; canvas?',
			!!canvas,
			'chartReady?',
			chartReady
		);
		...
		if (chart) {
			...
			if (!ChartCtor) return;
			console.info(
				'InstructorBreakdownChart: creating new Chart instance, instructors:',
				data.labels.length
			);
...
{#if !chartReady}
	<div
		style="height: 50vh; display:flex; align-items:center; justify-content:center; color: var(--text-secondary);"
	>
		<p>Loading chart...</p>
	</div>
{:else if !data || !data.datasets || data.datasets.length === 0 || !data.labels || data.labels.length === 0}
	<div
		style="height: 50vh; display:flex; align-items:center; justify-content:center; color: var(--text-secondary);"
	>
